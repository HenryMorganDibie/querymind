  ...
  async function testKey() {
    ...
    try {
      const text = await callLLM({
        ...
        systemPrompt: "You are a helpful assistant. Reply with only the word CONNECTED.",
        userMessage: "Test connection.",
      });
      if (text.toUpperCase().includes("CONNECTED") || text.length > 0) {
        setTestResult("ok");
...
function AnalystPage({ db, llmConfig, onBack }) {
  ...
  const ask = useCallback(async (question) => {
    ...
    try {
      const systemPrompt = `You are QueryMind — a senior data analyst, scientist, and business strategist combined into one. You think like a founder who has seen hundreds of businesses.

The user has this database schema:
${db.schema}

For every question you must:
1. Write correct, executable SQL for this schema (MySQL-compatible by default)
2. Generate realistic mock data the SQL WOULD return (max 15 rows, as a JSON array)
3. Extract 2–4 key metrics from the data that matter most
4. Write a headline that states the most important finding directly (not vague, not a question)
5. Write a detailed narrative (4–6 sentences) that:
   - States the main finding clearly upfront
   - Explains the "so what" — what does this mean for the business
   - Flags any caveats, risks, or things to investigate further
   - Is written like a senior analyst briefing a founder, not like a chatbot
6. Rate your confidence: "high" (schema clearly supports this), "medium" (some assumptions made), "low" (schema incomplete for this question)
...
Respond ONLY in valid JSON — no markdown, no backticks:
{
  ...
  "headline": "Direct statement of the main finding",
  "narrative": "Detailed analyst-grade interpretation...",
  "confidence": "high" | "medium" | "low",
  "vizType": "bar" | "line" | "area" | "pie" | "table" | "stat" | "stats"
}

Be honest. If the schema doesn't have enough information to answer something definitively, say so in the narrative. Never make up a finding that isn't in the data.`;
      ...
      setMessages(prev => [...prev, {
        ...
        role: "assistant",
        type: "error",
        text: "I ran into a problem generating that analysis. Try rephrasing your question or check that your schema has the tables needed to answer it.",
      }]);
    }
